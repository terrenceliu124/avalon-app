...
function computeRoleComposition(playerCount, selectedRoles) {
...
function RoleCompositionSummary({ playerCount, selectedRoles }) {
  const comp = computeRoleComposition(playerCount, selectedRoles);
  if (!comp) return null;
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
        {comp.goodPool.map((r, i) => (
          <span key={i} className="badge badge-good">{r}</span>
        ))}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {comp.evilPool.map((r, i) => (
          <span key={i} className="badge badge-evil">{r}</span>
        ))}
      </div>
    </div>
  );
    ...
    <div className="page" style={bgStyle}>
      ...
      {isHost ? (
        <div className="card" style={cardTexturedStyle}>
          ...
          <RoleCompositionSummary playerCount={playerCount} selectedRoles={room.selectedRoles} />

          <div style={{ borderTop: '1px solid #333', paddingTop: 14, marginTop: 14 }}>
            <h3 style={{ margin: '0 0 10px' }}>Optional Roles</h3>
          </div>
